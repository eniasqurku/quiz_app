...
def test_participant_register(random_user_data, api_client):
    participant_register_url = reverse('participant')
    response = api_client.post(participant_register_url, random_user_data, format='json')
...
def test_creator_register(random_user_data, api_client):
    participant_register_url = reverse('creator')
    response = api_client.post(participant_register_url, random_user_data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
