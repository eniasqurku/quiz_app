...
def test_creator_can_see_his_quizzes(api_client, creator_user, quizzes):
...
@pytest.mark.django_db
def test_participant_cant_see_any_quizzes_without_participating(
    api_client, participant_user, quizzes
):
    quizzes_url = reverse("quizzes")
    response = api_client.get(quizzes_url)
    assert response.status_code == status.HTTP_200_OK
...
@pytest.mark.django_db
def test_participant_can_only_see_quizzes_he_participated(
    api_client, participant_user, quizzes
):
    quizzes_url = reverse("quizzes")
    quiz_1 = quizzes[0]
    quiz_2 = quizzes[1]
    QuizAttempt.objects.create(quiz=quiz_1, participant=participant_user)
    QuizAttempt.objects.create(quiz=quiz_2, participant=participant_user)
    response = api_client.get(quizzes_url)
    assert response.status_code == status.HTTP_200_OK
