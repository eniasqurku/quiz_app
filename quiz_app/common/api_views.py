...
from quiz_app.common.action_logger import (
    ADDITION,
    log_addition,
    CHANGE,
    log_change,
...
class MyCreateAPIView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        except (ObjectDoesNotExist, Http404) as dne:
...
class MyListCreateAPIView(MyCreateAPIView, MyListAPIView):
    ...
    filter_serializer_class = None
    serializer_error_msg = (
        "'%s' should either include a `serializer_class` attribute, or override the "
                            ...
                            for unit_key, unit_value in item.items():
                                for arr_item in unit_value:
                                    response_message += "{}: {} ".format(
                                        unit_key, arr_item["message"]
                                    )
                        elif isinstance(item, str):
                            for item_key, item_val in value.items():
                                for arr_item in item_val:
                                    response_message += "{}: {} ".format(
                                        item_key, arr_item["message"]
                                    )
